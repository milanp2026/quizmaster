...
export const getLiveLobby = query({
  ...
  handler: async (ctx, args) => {
    ...
    const currentQuestion = gameState?.currentQuestionId ? await ctx.db.get(gameState.currentQuestionId) : null;
...
export const createQuestion = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("questions", {
      ...
      points: args.points,
      answerDurationSeconds: args.answerDurationSeconds,
...
export const hostCreateQuestion = mutation({
  args: {
    gameId: v.id("games"),
    ...
    explanation: v.optional(v.string()),
    speedBonus: v.boolean(),
    status: v.union(v.literal("draft"), v.literal("ready"), v.literal("publish")),
  },
  handler: async (ctx, args) => {
    ...
    if (args.type === "multiple_choice") {
      const options = [args.optionA, args.optionB, args.optionC, args.optionD].map((option) => option?.trim());
      if (options.some((option) => !option)) {
        throw new Error("Vul alle vier antwoorden in.");
      }
      if (!args.correctOption || !["A", "B", "C", "D"].includes(args.correctOption)) {
        throw new Error("Kies het juiste antwoord.");
      }
    }
    ...
    const questionId = await ctx.db.insert("questions", {
      ...
      optionD: args.optionD?.trim(),
      correctOption: args.correctOption,
      category: args.category?.trim() || "Algemeen",
      points: Math.max(0, args.points),
      answerDurationSeconds: Math.min(Math.max(Math.floor(args.answerDurationSeconds), 5), 300),
      explanation: args.explanation?.trim(),
      speedBonus: args.speedBonus,
      status: args.status === "publish" ? "ready" : args.status,
      createdAt: Date.now(),
    });

    if (args.status === "ready" || args.status === "publish") {
      ...
      await ctx.db.patch(args.gameId, { livePhase: "question_ready" });
    }

    if (args.status === "publish") {
      await startQuestionInternal(ctx, args.gameId, questionId);
    }
...
async function startQuestionInternal(ctx: MutationCtx, gameId: Id<"games">, questionId: Id<"questions">) {
  const question = await ctx.db.get(questionId);
  const gameState = await getGameStateByGameId(ctx, gameId);
  ...
  const durationSeconds = question.answerDurationSeconds ?? 20;
  ...
  await ctx.db.patch(gameId, {
    ...
    currentAnswerDeadline: startedAt + durationSeconds * 1000,
    currentQuestionNumber: (await ctx.db.get(gameId))?.currentQuestionNumber ?? 1,
  });
}

export const hostQueueQuestion = mutation({
  args: {
    gameId: v.id("games"),
    hostSessionToken: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    if (!question || question.gameId !== args.gameId) {
      throw new Error("Vraag niet gevonden.");
    }
    const gameState = await getGameStateByGameId(ctx, args.gameId);
    if (gameState) {
...
export const hostStartQuestion = mutation({
  args: {
    gameId: v.id("games"),
    hostSessionToken: v.string(),
    questionId: v.id("questions"),
  ...
  handler: async (ctx, args) => {
    await requireHostSession(ctx, args.gameId, args.hostSessionToken);
    await startQuestionInternal(ctx, args.gameId, args.questionId);
  },
});

export const hostCloseQuestion = mutation({
  args: {
    gameId: v.id("games"),
    hostSessionToken: v.string(),
  },
  handler: async (ctx, args) => {
    ...
    if (!gameState?.currentQuestionId) {
      throw new Error("Er is geen actieve vraag.");
    }
    ...
    await ctx.db.patch(args.gameId, { livePhase: "answering_closed" });
  },
});

export const hostRevealAnswer = mutation({
  args: {
    gameId: v.id("games"),
    hostSessionToken: v.string(),
  },
  handler: async (ctx, args) => {
    ...
    const gameState = await getGameStateByGameId(ctx, args.gameId);
    if (!gameState?.currentQuestionId) {
      throw new Error("Er is geen vraag om te tonen.");
    }
    ...
    await ctx.db.patch(args.gameId, { livePhase: "showing_answer" });
    ...
    if (question?.type === "multiple_choice" && question.correctOption) {
      ...
      for (const answer of answers) {
        ...
        await ctx.db.patch(answer.playerId, { score: player.score - previousPoints + nextPoints });
...
export const hostNextQuestion = mutation({
  args: {
    gameId: v.id("games"),
    hostSessionToken: v.string(),
  },
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.gameId, {
      livePhase: "waiting_for_question",
      currentQuestionNumber: (game?.currentQuestionNumber ?? 1) + 1,
      currentQuestionStartedAt: undefined,
      currentAnswerDeadline: undefined,
    });
  },
});
...
export const submitAnswer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const game = await ctx.db.get(args.gameId);
    if (game?.currentAnswerDeadline && game.currentAnswerDeadline < Date.now()) {
      throw new Error("De tijd is voorbij.");
    }
